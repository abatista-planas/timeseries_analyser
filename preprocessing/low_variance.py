...
def drop_low_variance_columns(
    ...
    always preserving the time column (and target column if provided).

    Args:
        df (pd.DataFrame): Input DataFrame.
        time_column (str): The column to always keep.
        target_column (str, optional): Another column to always keep.
        threshold (float): Minimum variance for a feature to be kept.

    Returns:
        pd.DataFrame: DataFrame with low-variance columns dropped.
    """
    cols_to_exclude = [time_column]
    if target_column:
        cols_to_exclude.append(target_column)

    # Only fit to numeric columns, excluding time/target
    feature_cols = [
        col
    ...
    # Columns to keep: time_column, (target_column), and high variance feature columns
